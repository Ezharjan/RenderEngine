...
namespace RenderEngine {
	...
	public:
		Vertex() {}
		Vertex(Vector4 p, Colour c) :position(p), color(c) {}
		Vertex(Vector4 p, Colour c, Vector3 t) :position(p), color(c), texcoord(t) {}
		Vertex(Vector4 p, Vector4 n, Vector3 t) :position(p), normal(n), texcoord(t) {}
		Vertex(Vector4 p, Vector4 n, Colour c, Vector3 t) :position(p), normal(n), color(c), texcoord(t) {}
		Vertex(const Vertex& v)
		{
			position = v.GetVertexPosition();
			normal = v.GetVertexNormal();
			color = v.GetVertexColour();
		...
		void SetVertexColour(const Colour& c) { color = c; }
		Colour GetVertexColour() const { return color; }

		void SetVertexTexcoord(const Vector3& t) { texcoord = t; }
		Vector3 GetVertexTexcoord() const { return texcoord; }

		inline Vertex ClampLerp(const Vertex &v1, const Vertex &v2, float factor)
		{
			CLAMP(factor, 0.0f, 1.0f);
			return Lerp(v1, v2, factor);
		}

		inline Vertex Lerp(const Vertex &v1, const Vertex &v2, float factor)
		{
			...
			Colour c = v1.GetVertexColour() + (v2.GetVertexColour() - v1.GetVertexColour()) * factor;
	...
	public:
		...
		inline Uniform ClampLerp(const std::shared_ptr<Uniform> &u1, const std::shared_ptr<Uniform> &u2, float factor)
		{
			CLAMP(factor, 0.0f, 1.0f);
			return Lerp(u1, u2, factor);
		}

		inline Uniform Lerp(const std::shared_ptr<Uniform> u1, const std::shared_ptr<Uniform> u2, float factor)
		{
